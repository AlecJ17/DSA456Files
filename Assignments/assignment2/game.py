...
class Board:
    def __init__(self,width,height, p1_sprites, p2_sprites):
        ...
        self.turn = 0
        self.move_history = Stack()  # Stack to store previous states and player type
    ...
    # Modify add_piece for undo function
    def add_piece(self, row, col, player, is_human):
        if self.valid_move(row, col, player):
            # Save the current state before making a move if it's a human player
            if is_human:
                current_state = [row[:] for row in self.board]
                self.move_history.push((current_state, self.turn))
    ...
    def undo_move(self):
        if not self.move_history.is_empty():
            previous_state, previous_turn = self.move_history.pop()
...
# Add a button class for undo button
class Button:
    def __init__(self, x, y, width, height, text, color_active, color_inactive):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
...
undo_button = Button(900, 200, 100, 50, 'Undo', (0, 200, 0), (200, 0, 0))
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        elif event.type == pygame.MOUSEBUTTONDOWN and undo_button.is_over(pygame.mouse.get_pos()):  # Undo button event
            if undo_button.active:
                board.undo_move()
        else:
    ...
    if not has_winner:
        if overflowing:
            ...
            if make_move:
                board.add_piece(grid_row, grid_col, player_id[current_player], choice[current_player] == 0) # Add choice[current_player] == 0 to indicate human player
